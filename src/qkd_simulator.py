...
class BB84Protocol:
    ...
    def prepare_quantum_states(self, bits: List[int], bases: List[str]) -> List[QuantumState]:
    ...
    def simulate_channel_transmission(self, states: List[QuantumState]) -> List[QuantumState]:
    ...
    def sift_key(self, alice_bits: List[int], alice_bases: List[str], 
                 bob_bases: List[str], bob_measurements: List[int]) -> Tuple[List[int], List[int]]:
        """Sift the key keeping only matching bases and successful measurements"""
        alice_sifted = []
        bob_sifted = []
        
        for a_bit, a_basis, b_basis, b_measurement in zip(alice_bits, alice_bases, bob_bases, bob_measurements):
    ...
    def privacy_amplification(self, key: List[int], compression_ratio: float = 0.8) -> List[int]:
    ...
    def run_protocol(self) -> Dict:
        ...
        results = {
            ...
            'qber': qber,
            'secure': secure,
            'sift_ratio': len(alice_sifted) / self.params.key_length if self.params.key_length > 0 else 0,
        ...
        logger.info(f"Protocol completed. QBER: {qber:.4f}, Final key length: {len(self.final_key)}")
        return results
...
class QKDSystemSimulator:
    ...
    def simulate_session(self, session_id: int = 0) -> Dict:
        ...
        if self.params.protocol == "BB84":
            ...
            raise NotImplementedError(f"Protocol {self.params.protocol} not implemented")
        ...
        for i in range(num_sessions):
            ...
            if random.random() < 0.1:  # 10% chance of failure
                failure_type = random.choice([
                    "channel_loss", "detector_noise", "timing_drift", 
                    "eavesdropping", "source_instability"
                ])
    ...
    def get_system_statistics(self) -> Dict:
        ...
        final_lengths = [session['final_key_length'] for session in self.simulation_history]
        ...
        stats = {
            ...
            'success_rate': sum(1 for session in self.simulation_history if session['secure']) / len(self.simulation_history)
        }
    ...
    def plot_performance_metrics(self, save_path: str = None):
        ...
        axes[1, 1].axvline(x=0.11, color='r', linestyle='--', label='Security Threshold')
        axes[1, 1].legend()
