...
class EavesdroppingDetector:
    ...
    def detect_intercept_resend_attack(self, session: Dict) -> Dict[str, Any]:
        ...
        if hasattr(self, "baseline_stats") and "sift_ratio" in self.baseline_stats:
            ...
            if (
                sift_ratio
                < baseline_sift - 2 * self.baseline_stats["sift_ratio"]["std"]
            ):
...
class RealTimeSecurityMonitor:
    ...
    def process_session(
        self,
        session: Dict,
        eavesdropper: EavesdroppingDetector,
        metrics_calc: SecurityMetricsCalculator,
    ) -> Dict[str, Any]:
        ...
        attack_detected = (
            intercept_result["attack_detected"]
            or beam_split_result["attack_detected"]
            or pns_result["attack_detected"]
        )
    ...
    def _calculate_overall_security_score(
        self, info_metrics: Dict, channel_metrics: Dict, protocol_metrics: Dict
    ) -> float:
        ...
        score += (
            weights["security_margin"] * channel_metrics.get("security_margin", 0) * 10
        )  # Scale margin

        # Channel security contribution
        score += weights["channel_fidelity"] * max(
            0, channel_metrics.get("channel_fidelity", 0)
        )
        score += weights["effective_transmission"] * channel_metrics.get(
            "effective_transmission", 0
        )
    ...
    def get_security_report(self, time_window: timedelta = None) -> Dict[str, Any]:
        ...
        if time_window:
            ...
            recent_logs = [
                log for log in self.security_log if log["timestamp"] > cutoff_time
            ]
            recent_alerts = [
                alert
        ...
        if len(security_scores) > 10:
            ...
            if recent_avg < overall_avg * 0.9:
                recommendations.append(
                    "Security performance is declining - investigate system degradation"
                )

        # Check attack frequency
        if np.mean(attack_rates) > 0.1:
            recommendations.append(
                "High attack detection rate - enhance security monitoring"
            )

        # Check specific metrics across sessions
        channel_fidelities = [
            log["security_metrics"]["channel"]["channel_fidelity"]
            for log in logs
            if "channel_fidelity" in log["security_metrics"]["channel"]
        ]

        if channel_fidelities and np.mean(channel_fidelities) < 0.8:
            recommendations.append(
                "Low channel fidelity detected - check optical components"
            )

        if not recommendations:
            recommendations.append(
                "System security appears nominal - continue regular monitoring"
            )
...
class QKDSecuritySystem:
    ...
    def monitor_session(self, session: Dict) -> Dict[str, Any]:
        ...
        return self.monitor.process_session(
            session, self.eavesdropper, self.metrics_calculator
        )
    ...
    def plot_security_analysis(
        self, analysis_results: List[Dict], save_path: str = None
    ):
        ...
        security_scores = [
            result["overall_security_score"] for result in analysis_results
        ]
        ...
        secret_key_rates = [
            result["security_metrics"]["information"]["secret_key_rate"]
        ...
        channel_fidelities = [
            result["security_metrics"]["channel"]["channel_fidelity"]
            for result in analysis_results
        ]
        ...
        security_margins = [
            result["security_metrics"]["channel"]["security_margin"]
            for result in analysis_results
        ]
    ...
    def generate_security_report(self) -> str:
        ...
        report_text.append(
            f"Mean Security Score: {security['mean_security_score']:.3f}"
        )
        report_text.append(
            f"Minimum Security Score: {security['min_security_score']:.3f}"
        )
        report_text.append(
            f"Attack Detection Rate: {security['attack_detection_rate']:.1%}"
        )
        report_text.append(
            f"Total Attacks Detected: {security['total_attacks_detected']}"
        )
        report_text.append("")
