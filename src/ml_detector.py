...
class FeatureEngineer:
    ...
    def extract_temporal_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [5, 10, 20]:
            features[f'qber_mean_{window}'] = features['qber'].rolling(window=window).mean()
            features[f'qber_std_{window}'] = features['qber'].rolling(window=window).std()
            features[f'sift_ratio_mean_{window}'] = features['sift_ratio'].rolling(window=window).mean()
            features[f'key_efficiency_trend_{window}'] = features['key_efficiency'].rolling(window=window).apply(
                lambda x: float(np.polyfit(range(len(x)), x, 1)[0]) if len(x) == window and not x.isna().any() else 0.0,
                raw=False
            )
        ...
        for lag in [1, 2, 5]:
            ...
            features[f'key_efficiency_lag_{lag}'] = features['key_efficiency'].shift(lag)
    ...
    def extract_statistical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['qber_efficiency_ratio'] = features['qber'] / (features['key_efficiency'] + 1e-8)
        
        # Normality tests (simplified)
        for window in [10, 20]:
            features[f'qber_skewness_{window}'] = features['qber'].rolling(window=window).skew()
            features[f'qber_kurtosis_{window}'] = features['qber'].rolling(window=window).kurt()
        ...
        # Efficiency ratios
        features['theoretical_efficiency'] = 1 - 2 * features['qber']  # Theoretical maximum
        features['efficiency_gap'] = features['theoretical_efficiency'] - features['key_efficiency']
    ...
    def extract_domain_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['entropy_rate'] = -features['qber'] * np.log2(features['qber'] + 1e-8) - \
        ...
        if 'channel_loss' in features.columns:
            features['channel_quality'] = features['sift_ratio'] * (1 - features['channel_loss'])
        else:
            features['channel_quality'] = features['sift_ratio']  # Use sift_ratio as proxy
            
        if 'initial_length' in features.columns:
            features['effective_transmission'] = features['final_key_length'] / features['initial_length']
        else:
            features['effective_transmission'] = features['final_key_length'] / 1000.0  # Assume 1000 initial length
        ...
        # Throughput metrics
        features['bit_throughput'] = features['final_key_length'] / (1 + features['session_id'])  # Simplified time
        
        # Handle normalized throughput with fallback
        if 'sifted_length' in features.columns:
            features['normalized_throughput'] = features['final_key_length'] / (features['sifted_length'] + 1e-8)
        else:
            # Fallback calculation using sift_ratio if sifted_length not available
            estimated_sifted = features['initial_length'] * features.get('sift_ratio', 0.5)
            features['normalized_throughput'] = features['final_key_length'] / (estimated_sifted + 1e-8)
    ...
    def prepare_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Store feature names
        self.feature_names = [col for col in features.columns if col not in ['session_id']]
    ...
    def fit(self, features: pd.DataFrame, labels: np.ndarray = None):
        ...
        for i in range(X.shape[0]):
            for j in range(X.shape[1]):
                if hasattr(X[i, j], '__iter__') and not isinstance(X[i, j], (str, bytes)):
...
class MLDetectionSystem:
    ...
    def detect_failures(self, test_data: List[Dict]) -> Dict:
        ...
        results = {
            ...
            'session_ids': features['session_id'].values if 'session_id' in features.columns else np.arange(len(features))
        }
        
        return results
    
    def evaluate_performance(self, test_data: List[Dict], true_labels: np.ndarray = None) -> Dict:
    ...
    def plot_ml_analysis(self, results: Dict, save_path: str = None):
        ...
        if 'random_forest' in classifications:
            ...
            for i, label in enumerate(unique_labels):
                ...
                if np.any(mask):
                    axes[0, 0].scatter(features['qber'][mask], features['sift_ratio'][mask], 
        ...
        axes[0, 1].scatter(features['qber'][~combined_anomalies], features['sift_ratio'][~combined_anomalies], 
        ...
        if hasattr(self.failure_classifier, 'feature_importance') and 'random_forest' in self.failure_classifier.feature_importance:
        ...
        anomaly_counts = [np.sum(anomalies[method]) for method in anomaly_methods if method in anomalies]
        
        if anomaly_counts:
            axes[1, 1].bar(range(len(anomaly_counts)), anomaly_counts)
            axes[1, 1].set_xticks(range(len(anomaly_counts)))
            axes[1, 1].set_xticklabels([method for method in anomaly_methods if method in anomalies], rotation=45)
        ...
        if 'random_forest' in classifications:
            ...
            numeric_labels = label_encoder.fit_transform(classifications['random_forest'])
    ...
    def train(self, X, y, model_type='random_forest', **kwargs):
        ...
        self.model = self.failure_classifier.models.get(model_type, 
                                                       list(self.failure_classifier.models.values())[0])
    ...
    def evaluate(self, X_test, y_test):
        ...
        for i in range(len(X_test)):
            sample = X_test.iloc[i:i+1] if isinstance(X_test, pd.DataFrame) else pd.DataFrame([X_test[i]])
            result = self.predict(sample)
            predictions.append(result.prediction)
            probabilities.append(result.probabilities[1] if len(result.probabilities) > 1 else 0.5)
    ...
    def optimize_hyperparameters(self, X_train, y_train, model_type='random_forest', param_grid=None, cv=3):
    ...
    def get_feature_importance(self):
        ...
        feature_names = getattr(self, 'feature_names_', [f'feature_{i}' for i in range(len(importance_scores))])
