...
class StatisticalAnomalyDetector:
    ...
    def establish_baseline(self, data: np.ndarray, metric_name: str):
        ...
        logger.info(f"Baseline established for {metric_name}: mean={mean:.4f}, std={std:.4f}")
    ...
    def control_chart_analysis(self, data: np.ndarray, metric_name: str) -> Dict[str, np.ndarray]:
    ...
    def _detect_trend_violations(self, data: np.ndarray, trend_length: int = 7) -> np.ndarray:
    ...
    def _detect_run_violations(self, data: np.ndarray, metric_name: str, run_length: int = 8) -> np.ndarray:
...
class QKDAnomalyDetector:
    ...
    def establish_baseline(self, training_data: List[Dict]):
        ...
        # Fit ML models
        ml_features = features_df[['qber', 'sift_ratio', 'key_efficiency', 
                                  'channel_loss', 'error_rate', 'error_amplification']].values
    ...
    def cusum_analysis(self, data: np.ndarray, target: float = None, h: float = 5.0, k: float = 0.5) -> Dict:
    ...
    def detect_outliers_modified_zscore(self, data: np.ndarray, threshold: float = 3.5) -> List[int]:
        ...
        for method in methods:
            if method in results:
                if method == 'statistical':
                    ...
                    stat_scores = [np.mean(anomalies.astype(float)) 
                    ...
                    domain_scores = [np.mean(anomalies.astype(float)) 
    ...
    def detect_anomalies(self, test_data: List[Dict]) -> Dict:
        ...
        for metric in ['qber', 'sift_ratio', 'final_key_length', 'key_efficiency']:
            if metric in features_df.columns:
                ...
                statistical_anomalies[f'{metric}_zscore'] = self.statistical_detector.detect_outliers_zscore(data, metric)
                statistical_anomalies[f'{metric}_iqr'] = self.statistical_detector.detect_outliers_iqr(data, metric)
                
                # Control chart analysis
                control_violations = self.statistical_detector.control_chart_analysis(data, metric)
    ...
    def _detect_domain_specific_anomalies(self, features_df: pd.DataFrame) -> Dict[str, np.ndarray]:
        """Detect domain-specific QKD anomalies"""
        anomalies = {}
        
        # QBER security threshold violation
        anomalies['qber_security_violation'] = features_df['qber'] > self.config['qber_threshold']
        ...
        anomalies['low_sift_ratio'] = features_df['sift_ratio'] < self.config['sift_ratio_threshold']
        
        # Final key length too short
        anomalies['short_key_length'] = features_df['final_key_length'] < self.config['key_length_threshold']
        ...
        if len(features_df) > 1:
            ...
            anomalies['sudden_qber_increase'] = np.concatenate([[False], qber_diff > 0.05])
            anomalies['sudden_sift_decrease'] = np.concatenate([[False], sift_diff < -0.2])
        else:
    ...
    def _calculate_overall_anomaly_score(self, statistical: Dict, ml: Dict, domain: Dict) -> np.ndarray:
        ...
        domain_count = sum(anomalies.astype(int) for anomalies in domain.values()) * 2  # Weight domain knowledge
    ...
    def plot_anomaly_analysis(self, results: Dict, save_path: str = None):
        ...
        axes[0, 0].scatter(features_df['session_id'][overall_anomaly > 0.3], 
                          features_df['qber'][overall_anomaly > 0.3], 
                          color='red', s=50, label='Anomalies')
        axes[0, 0].axhline(y=self.config['qber_threshold'], color='r', linestyle='--', 
        ...
        axes[0, 1].plot(features_df['session_id'], features_df['sift_ratio'], 'g-', alpha=0.7)
        axes[0, 1].scatter(features_df['session_id'][overall_anomaly > 0.3], 
        ...
        axes[1, 0].axhline(y=0.3, color='orange', linestyle='--', label='Anomaly Threshold')
        ...
        # Key efficiency
        axes[1, 1].plot(features_df['session_id'], features_df['key_efficiency'], 'm-', alpha=0.7)
        axes[1, 1].scatter(features_df['session_id'][overall_anomaly > 0.3], 
        ...
        anomaly_types = []
        for method, anomalies_dict in [('Statistical', results['statistical']), 
        ...
        axes[2, 1].axvline(x=np.percentile(ml_scores, 90), color='red', linestyle='--', 
    ...
    def generate_anomaly_report(self, results: Dict) -> str:
        ...
        report.append("=" * 50)
        report.append(f"Analysis Period: Sessions {features_df['session_id'].min()} to {features_df['session_id'].max()}")
        ...
        report.append(f"Anomaly Rate: {np.sum(overall_anomaly > 0.3) / len(features_df) * 100:.2f}%")
        report.append("")
        
        # Statistical summary
        report.append("STATISTICAL SUMMARY:")
        report.append("-" * 20)
        report.append(f"Mean QBER: {features_df['qber'].mean():.4f} ± {features_df['qber'].std():.4f}")
        report.append(f"Mean Sift Ratio: {features_df['sift_ratio'].mean():.4f} ± {features_df['sift_ratio'].std():.4f}")
        report.append(f"Mean Key Efficiency: {features_df['key_efficiency'].mean():.4f} ± {features_df['key_efficiency'].std():.4f}")
        report.append("")
        ...
        if qber_violations > 0:
            ...
            report.append("⚠️  IMMEDIATE ATTENTION REQUIRED - Potential security breach detected!")
            report.append("")
        ...
        if np.any(overall_anomaly > 0.3):
            ...
            if np.sum(high_anomaly_mask) > 0:
                ...
                for idx in reversed(top_indices):
                    ...
                    report.append(f"Session {session_id}: Anomaly Score = {score:.3f}, QBER = {qber:.4f}")
