...
class QKDSignalProcessor:
    ...
    def simulate_quantum_signal(
        self, qkd_session: Dict, noise_level: float = 0.1
    ) -> np.ndarray:
        ...
        signal_with_noise[error_spikes] += np.random.normal(
            0, 0.5, np.sum(error_spikes)
        )
    ...
    def _spectral_entropy(self, signal: np.ndarray) -> float:
        ...
        freqs, psd = scipy.signal.welch(
            signal, fs=self.sampling_rate, nperseg=min(256, len(signal) // 4)
        )
...
class FrequencyAnalyzer:
    ...
    def power_spectral_density(
        ...
        freqs, psd = scipy.signal.welch(
            signal, fs=self.sampling_rate, nperseg=nperseg, noverlap=int(nperseg * 0.5)
        )
        return freqs, psd
    ...
    def spectrogram_analysis(
        ...
        freqs, times, Sxx = scipy.signal.spectrogram(
            signal, fs=self.sampling_rate, nperseg=min(256, len(signal) // 8)
        )
    ...
    def extract_frequency_features(self, signal: np.ndarray) -> Dict[str, float]:
        ...
        features["spectral_centroid"] = np.sum(freqs * fft_magnitude) / np.sum(
            fft_magnitude
        )

        # Spectral spread (standard deviation around centroid)
        features["spectral_spread"] = np.sqrt(
            np.sum(((freqs - features["spectral_centroid"]) ** 2) * fft_magnitude)
            / np.sum(fft_magnitude)
        )

        # Spectral rolloff (frequency below which 85% of energy is contained)
        cumsum = np.cumsum(fft_magnitude)
        features["spectral_rolloff"] = freqs[
            np.where(cumsum >= 0.85 * cumsum[-1])[0][0]
        ]

        # Spectral flux (measure of how quickly the power spectrum changes)
        if len(self.signal_history) > 0:
            prev_spectrum = np.abs(np.fft.fft(self.signal_history[-1]))[
                : len(fft_magnitude)
            ]
        ...
        if len(bandwidth_indices) > 0:
            features["bandwidth"] = (
                freqs[bandwidth_indices[-1]] - freqs[bandwidth_indices[0]]
            )
        else:
...
class AnomalyPatternDetector:
    ...
    def establish_baseline(
        self, normal_signals: List[np.ndarray], processor: QKDSignalProcessor
    ):
    ...
    def detect_signal_anomalies(
        self, signal: np.ndarray, processor: QKDSignalProcessor
    ) -> Dict[str, bool]:
    ...
    def detect_pattern_anomalies(
        self, signals: List[np.ndarray], processor: QKDSignalProcessor
    ) -> Dict:
        ...
        for feature_name in feature_df.columns:
            ...
            if len(values) > 5:
                ...
                baseline_std = self.baseline_features.get(feature_name, {}).get(
                    "std", 1
                )

                pattern_anomalies[f"{feature_name}_strong_trend"] = (
                    abs(trend_coeff) > 2 * baseline_std
                )
...
class SignalQualityAnalyzer:
    ...
    def calculate_snr(
        self, signal: np.ndarray, noise_estimate: np.ndarray = None
    ) -> float:
    ...
    def _lowpass_filter(
        self, signal: np.ndarray, cutoff: float, order: int = 5
    ) -> np.ndarray:
    ...
    def analyze_signal_quality(
        self, signal: np.ndarray, qkd_params: Dict
    ) -> Dict[str, float]:
        ...
        # Dynamic range
        quality_metrics["dynamic_range"] = 20 * np.log10(
            np.max(signal) / np.min(np.abs(signal[signal != 0]))
        )

        # Peak-to-average power ratio
        peak_power = np.max(signal**2)
        avg_power = np.mean(signal**2)
        quality_metrics["papr_db"] = (
            10 * np.log10(peak_power / avg_power) if avg_power > 0 else 0
        )

        # Effective number of bits (ENOB)
        noise_floor = np.std(signal) / np.sqrt(2)  # RMS noise
        full_scale = np.max(signal) - np.min(signal)
        quality_metrics["enob"] = (
            np.log2(full_scale / (2 * noise_floor)) if noise_floor > 0 else 16
        )
        ...
        quality_metrics["sfdr_db"] = (
            20 * np.log10(signal_peak / spurious_peak) if spurious_peak > 0 else 100
        )

        # Total harmonic distortion
        quality_metrics["thd"] = self.calculate_thd(
            signal, 10.0
        )  # Assuming 10 Hz fundamental
...
class QKDSignalAnalyzer:
    ...
    def analyze_qkd_session(self, qkd_session: Dict) -> Dict:
        ...
        quality_metrics = self.quality_analyzer.analyze_signal_quality(
            signal, qkd_session
        )
        ...
        if len(results) > 5:
            ...
            for i, result in enumerate(results):
                result["pattern_anomalies"] = {
                    ...
                    for key, value in pattern_anomalies.items()
                }
    ...
    def plot_signal_analysis(self, analysis_results: List[Dict], save_path: str = None):
        ...
        for i, result in enumerate(analysis_results[:5]):  # Plot first 5 signals
            axes[0, 0].plot(
                result["signal"][:500],
                alpha=0.7,
                label=f"Session {result['session_id']}",
            )
        ...
        if analysis_results:
            freqs, fft_mag = self.frequency_analyzer.fft_analysis(
                analysis_results[0]["signal"]
            )
        ...
        # SNR over sessions
        snr_values = [
            result["quality_metrics"]["snr_db"] for result in analysis_results
        ]
        ...
        time_features_df = pd.DataFrame(
            [result["time_features"] for result in analysis_results]
        )
        ...
        quality_df = pd.DataFrame(
            [result["quality_metrics"] for result in analysis_results]
        )
        ...
        spectral_centroids = [
            result["frequency_features"]["spectral_centroid"]
        ...
        if analysis_results[0]["signal_anomalies"]:
            ...
            if anomaly_counts:
                axes[2, 0].bar(
                    range(len(anomaly_counts)), list(anomaly_counts.values())
                )
                axes[2, 0].set_xticks(range(len(anomaly_counts)))
                axes[2, 0].set_xticklabels(
                    list(anomaly_counts.keys()), rotation=45, ha="right"
                )
        ...
        if len(analysis_results) >= 2:
            freqs1, psd1 = self.frequency_analyzer.power_spectral_density(
                analysis_results[0]["signal"]
            )
            freqs2, psd2 = self.frequency_analyzer.power_spectral_density(
                analysis_results[-1]["signal"]
            )
...
if __name__ == "__main__":
    ...
    for result in analysis_results[:3]:
        ...
        print(
            f"  Spectral Centroid: {result['frequency_features']['spectral_centroid']:.2f} Hz"
        )
