...
class TestDataGenerator:
    ...
    def generate_normal_qkd_session(self, session_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'detection_efficiency': np.random.normal(self.config.test_detection_efficiency, 0.02),
            'secure': True,
            'protocol': 'BB84',
    ...
    def generate_anomalous_qkd_session(self, session_id: int, anomaly_type: str) -> Dict[str, Any]:
    ...
    def generate_test_sessions(self) -> List[Dict[str, Any]]:
        ...
        sessions = []
        anomaly_types = ['high_qber', 'low_sift_ratio', 'channel_attack', 'detector_malfunction', 'eavesdropping']
        
        num_anomalies = int(self.config.num_test_sessions * self.config.anomaly_ratio)
        anomaly_indices = np.random.choice(self.config.num_test_sessions, num_anomalies, replace=False)
        ...
        for i in range(self.config.num_test_sessions):
            if i in anomaly_indices:
                ...
                session = self.generate_normal_qkd_session(self.config.num_baseline_sessions + i)
    ...
    def generate_time_series_data(self, num_points: int = 1000) -> Dict[str, np.ndarray]:
...
class MockQKDHardware:
    ...
    def measure_session(self, session_id: int) -> Dict[str, Any]:
        ...
        if self.error_simulation:
            return generator.generate_anomalous_qkd_session(session_id, 'detector_malfunction')
        else:
    ...
    def scenario_eavesdropping_attack() -> Dict[str, Any]:
        ...
        for i in range(config.num_test_sessions):
            if i < int(config.num_test_sessions * config.anomaly_ratio):
                ...
                session = generator.generate_normal_qkd_session(config.num_baseline_sessions + i)
                session['anomaly_type'] = 'normal'
                session['is_anomaly'] = False
    ...
    def scenario_hardware_degradation() -> Dict[str, Any]:
        ...
        for i in range(config.num_test_sessions):
            ...
            if i > config.num_test_sessions // 2:
                ...
                session['final_key_length'] = int(session['final_key_length'] * degradation_factor)
...
class TestDataExporter:
    ...
    @staticmethod
    def create_test_report(scenario: Dict[str, Any], results: Dict[str, Any], filename: str):
