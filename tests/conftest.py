    ...
    data = pd.DataFrame(
        {
            ...
            "detector_efficiency": np.concatenate(
                [normal_detector_eff, anomalous_detector_eff]
            ),
            "channel_loss": np.concatenate(
                [normal_channel_loss, anomalous_channel_loss]
            ),
...
def sample_photon_stream(random_seed):
    ...
    inter_arrival_times = np.random.exponential(
        1 / detection_rate, int(duration * detection_rate * 1.5)
    )
...
def assert_signal_properties(signal, expected_properties):
    ...
    if "frequency_content" in expected_properties:
        ...
        freqs = np.fft.fftfreq(
            len(signal), 1 / expected_properties.get("sampling_rate", 1000)
        )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "ml: marks tests as machine learning related")
