...
class TestAnomalyDetector(unittest.TestCase):
    ...
    def test_control_chart_analysis(self):
        ...
        violations = self.detector.control_chart_analysis(self.anomaly_data, 'test_metric')
...
class TestMLDetector(unittest.TestCase):
    ...
    def test_feature_engineering(self):
        ...
        rolling_cols = [col for col in features.columns if 'mean_' in col or 'std_' in col]
    ...
    def test_data_preparation(self):
        ...
        self.assertIsInstance(features, pd.DataFrame)
        self.assertGreater(len(features.columns), len(self.sample_data[0]))  # Should have engineered features
...
class TestSignalAnalyzer(unittest.TestCase):
    ...
    def test_signal_simulation(self):
        ...
        signal = self.analyzer.signal_processor.simulate_quantum_signal(self.sample_session)
...
class TestSecurityMonitor(unittest.TestCase):
    ...
    def test_intercept_resend_detection(self):
        ...
        result_secure = self.eavesdrop_detector.detect_intercept_resend_attack(self.secure_session)
        self.assertFalse(result_secure['attack_detected'])
        
        # Test with insecure session
        result_insecure = self.eavesdrop_detector.detect_intercept_resend_attack(self.insecure_session)
    ...
    def test_beam_splitting_detection(self):
        ...
        result = self.eavesdrop_detector.detect_beam_splitting_attack(self.insecure_session)
...
class TestUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_qkd_data = [
            {'qber': 0.02, 'sift_ratio': 0.5, 'final_key_length': 800, 'initial_length': 1000},
            {'qber': 0.03, 'sift_ratio': 0.48, 'final_key_length': 750, 'initial_length': 1000},
            {'qber': -0.01, 'sift_ratio': 1.5, 'final_key_length': -100, 'initial_length': 0}  # Invalid
        ]
    ...
    def test_normalization(self):
        ...
        self.assertAlmostEqual(normalized['feature1'].std(ddof=0), 1, places=5)  # Use population std
    ...
    def test_autocorrelation_calculation(self):
        ...
        self.assertAlmostEqual(autocorr[0], 1.0, places=5)  # Perfect correlation at lag 0 (with tolerance)
...
def run_performance_benchmarks():
    ...
    detector = QKDAnomalyDetector()
    normal_data = [{'qber': 0.02, 'sift_ratio': 0.5, 'final_key_length': 800, 'initial_length': 1000}] * 100
