...
from security_monitor import SecurityMonitor, ThreatDetector, CryptographicVerifier, SecurityEvent
...
class TestSecurityMonitor:
    ...
    def test_anomaly_detection_algorithms(self):
        ...
        ml_results = self.threat_detector.ml_anomaly_detection(
            self.all_events
        )
        ...
        ts_results = self.threat_detector.time_series_anomaly_detection(
            self.all_events
        )
    ...
    def create_beam_splitting_attack(self):
        ...
        return [
            SecurityEvent(
                ...
                detection_efficiency=0.6 + 0.05 * np.random.randn(),  # Reduced efficiency
                source='alice',
                severity='high'
            ) for i in range(50)
        ]
    ...
    def create_pns_attack(self):
        ...
        return [
            SecurityEvent(
                ...
                multi_photon_probability=0.15 + 0.02 * np.random.randn(),  # High multi-photon
                source='alice',
                severity='medium'
            ) for i in range(50)
        ]
    ...
    def test_key_integrity_verification(self):
        ...
        correlation = self.crypto_verifier.test_key_correlation(key_material, previous_key)
    ...
    def test_forensic_analysis(self):
        ...
        forensic_report = self.monitor.conduct_forensic_analysis(
            self.attack_events,
            analysis_window=timedelta(minutes=10)
        )
    ...
    def test_threat_detection_benchmark(self, benchmark):
        ...
        result = benchmark(
            self.threat_detector.analyze_events,
            self.all_events
        )
