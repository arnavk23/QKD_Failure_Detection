...
class TestSignalAnalyzer:
    ...
    def create_test_signals(self):
        ...
        # Chirp signal
        self.chirp_signal = signal.chirp(
            self.t, f0=10, f1=100, t1=self.duration, method="linear"
        )
    ...
    def create_qkd_signals(self):
        ...
        self.qber_signal = (
            base_qber + qber_variation + 0.005 * np.random.randn(len(self.t))
        )

        # Key rate signal
        base_key_rate = 1000
        key_rate_variation = 200 * np.sin(2 * np.pi * 0.05 * self.t)
        self.key_rate_signal = (
            base_key_rate + key_rate_variation + 50 * np.random.randn(len(self.t))
        )
    ...
    def test_qkd_specific_analysis(self):
        ...
        # Analyze QBER signal
        qber_analysis = self.analyzer.analyze_qber_signal(self.qber_signal, self.fs)
    ...
    def test_windowing_functions(self):
        ...
        for window_type in windows:
            windowed_signal = self.analyzer.apply_window(self.clean_signal, window_type)

            assert len(windowed_signal) == len(self.clean_signal)

            # Windowed signal should have reduced edge effects
            edge_ratio = np.mean(np.abs(windowed_signal[:10])) / np.mean(
                np.abs(windowed_signal)
            )
    ...
    def test_anomaly_detection(self):
        ...
        anomalous_signal[drift_start:] += np.linspace(
            0, 2, len(anomalous_signal) - drift_start
        )
    ...
    def test_fft_benchmark(self, benchmark):
        ...
        result = benchmark(self.analyzer.compute_fft, self.multi_freq_signal)
    ...
    def test_feature_extraction_benchmark(self, benchmark):
        ...
        result = benchmark(self.analyzer.extract_features, self.noisy_signal, self.fs)
...
class TestSpectralAnalyzer:
    ...
    def setup_method(self):
        ...
        self.test_signal = np.sin(2 * np.pi * 100 * self.t) + 0.5 * np.sin(
            2 * np.pi * 250 * self.t
        )
