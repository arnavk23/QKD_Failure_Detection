...
def main():
    ...
    for failure_type, intensity, count in failure_types:
        ...
        simulator.params = QKDParameters(
            key_length=1500, error_rate=0.015, detection_efficiency=0.85, 
            channel_loss=0.04, protocol="BB84"
        )
    ...
    test_simulator = QKDSystemSimulator(QKDParameters(
        key_length=1000, error_rate=0.02, detection_efficiency=0.8, 
        channel_loss=0.05, protocol="BB84"
    ))
    ...
    print(f"   Combined anomaly detection: {np.sum(combined_anomalies)} anomalies detected")
    print()
    ...
    ml_system.plot_ml_analysis(results, save_path='../plots/ml_performance/demo_ml_analysis.png')
    ...
    print(f"    Verification: Processed {len(verify_result['features'])} sessions with loaded models")
    print()
