...
def main():
    ...
    for i, (result, label) in enumerate(zip(analysis_results, session_labels)):
        ...
        anomaly_count = sum(result['signal_anomalies'].values()) if result['signal_anomalies'] else 0
    ...
    for quality, count in quality_summary.items():
        ...
        print(f"     {quality.capitalize():<10}: {count:3d} sessions ({percentage:5.1f}%)")
    print()
    ...
    analyzer.plot_signal_analysis(analysis_results, save_path='../plots/signal_analysis/demo_signal_analysis.png')
    ...
    for i, session in enumerate(monitoring_sessions):
        ...
        print(f"   Session {i+1}: SNR = {snr:5.1f} dB, Anomalies = {anomaly_count}, Status = {status}")
