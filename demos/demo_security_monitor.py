...
def main():
    ...
    print(f"   Baseline established from {baseline_stats['total_sessions']} secure sessions")
    print(f"   Baseline QBER: {baseline_stats['mean_qber']:.4f} ± {baseline_stats['std_qber']:.4f}")
    ...
    attacks_detected = sum(1 for result in security_results if result['attack_detected'])
    high_confidence = sum(1 for result in security_results if result['max_confidence'] > 0.8)
    ...
    for i, (result, label) in enumerate(zip(security_results, threat_labels)):
        ...
        detection_by_threat[label]['avg_security_score'].append(result['overall_security_score'])
    ...
    for threat, stats in detection_by_threat.items():
        ...
        print(f"     Detection Rate: {detection_rate:5.1f}% ({stats['detected']}/{stats['total']})")
    ...
    for result in security_results:
        for attack_type, attack_result in result['attack_types'].items():
            ...
            if attack_result['attack_detected']:
                ...
                attack_signatures[attack_type]['total_confidence'] += attack_result['confidence']
    ...
    for attack_type, sig in attack_signatures.items():
        if sig['detections'] > 0:
            ...
            sorted_indicators = sorted(sig['indicators'].items(), key=lambda x: x[1], reverse=True)
    ...
    predictions = np.array([1 if result['attack_detected'] else 0 for result in security_results])
    ...
    if detection_by_threat.get('intercept_resend', {}).get('detected', 0) < \
